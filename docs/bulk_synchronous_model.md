...
2.  **Global Memory Communication:** PEs exchange data via global load and store operations. The hardware internally utilizes the 2D mesh to route these requests.
...
| (Local SIMD/SRAM)   | (Global Load/Store)     | (Barrier) |
^                    ^                         ^           ^
...
- **Communicate Phase:** PEs issue `LDR_GLOBAL` and `STR_GLOBAL` operations. The hardware transparently routes these via the mesh. Packets may remain in-flight across the mesh during this phase.
...
| **Communicate** | `LDR_GLOBAL`, `STR_GLOBAL` |
| **Barrier** | `SYNC` |
...
# Deadlock Avoidance and Routing
The global memory model removes the need for the programmer to manage explicit send/receive symmetry. The hardware global memory unit manages request-response pairs and routing to ensure that no circular dependencies create deadlocks on the mesh. The `SYNC` instruction remains critical to ensure global consistency between supersteps.
